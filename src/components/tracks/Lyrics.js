...
const Lyrics = props => {
  ...
  useEffect(() => {
    axios
      .get(
        `https://localhost:3000/http://api.musixmatch.com/ws/1.0/track.lyrics.get?track_id=${props.match.params.id}
      ...
      .then(res => {
        ...
        return axios.get(
          `https://localhost:3000/http://api.musixmatch.com/ws/1.0/track.get?track_id=${props.match.params.id}
          &apikey=${process.env.REACT_APP_KEY}`
        );
      })
      .then(res => {
